...
The meaning of the classes is as follows:
 ...
 - **data.frame**: an object that inherits from `data.frame`, tolerate NAs. Option `No NA` throws an error if it contains NAs.
 
 - **vdata.frame**: an object that inherits from `data.frame`, or a vector; tolerate NAs. Option `No NA` throws an error if it contains NAs.
...
Let's illustrate when using `check_arg_plus`:
...
plot_cor = function(x, y = NULL, line.opts = list(), ...){
  ...
  check_arg(lm.opts, line.opts, "named list L0")
...
Now that we know how to use `check_arg`, let's provide some error-handling to the `lm` function. There is still need to check some stuff after that, but the bulk of the job is done:
